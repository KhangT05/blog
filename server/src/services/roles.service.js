...
const store = async (name, guard_name, description, permission = []) => {
    if (!permission || !Array.isArray(permission) || permission.length === 0) {
        throw new Error('Phải chọn ít nhất một quyền.');
    }
    ...
    if (validPermissions.length !== permission.length) {
        throw new Error('Một hoặc nhiều permission không hợp lệ.');
    }
